...
export default function Expenses() {
  ...
  async function loadData() {
    const e = await getExpenses('main', 25)
    setExpenses(e)
    setLoading(false)
  }
  ...
  return (
    <div className="space-y-8">
      <section>
        <h1 className="text-3xl font-semibold tracking-tight text-ink">Expenses</h1>
        <p className="mt-2 text-sm text-ink-secondary">
          Record money spent. Everyone can see the totals on the home page.
        </p>
      </section>
      ...
      <section className="grid gap-8 lg:grid-cols-2">
        <Card className="p-6">
          ...
          <ExpenseForm onSaved={loadData} />
        </Card>

        <Card className="p-6">
          <SectionHeading title="Recent expenses" />
          <TransactionList items={expenses} kind="expenses" loading={loading} />
        </Card>
      </section>
    </div>
  )
